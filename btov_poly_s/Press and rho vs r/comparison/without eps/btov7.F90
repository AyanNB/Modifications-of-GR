...
subroutine make_a_tov_star
  ...
  ! perform the integration
     ...
     do l=1,isurf
     write(un,"(i8,1P10E15.6)") l,rs(l)/1.0d5,mgrav(l)/msun,mrest(l)/msun,press_e(l)/press(1),rho(l)/rho_cit,eff_r(l)/rho_cit
     enddo
     close(unit=un)  
   enddo      
...
subroutine tov_integrate
  ...
  do i=2,nzones
     ...
     eff_r(i)= rho_crit/2*(1-(1-4*rho(i)/rho_crit)**0.5)
     print*,i,eff_r(i),rho_crit
  enddo
