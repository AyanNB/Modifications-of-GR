module tovmod1
  ...
  real*8  :: rmax  = 1.0d0
  real*8  :: dr = 0.0d0
  real*8  :: rho_cit=1.0d0,rho_crit=1.0d0

  real*8,allocatable  :: rs(:)
  real*8,allocatable  :: ri(:)
  real*8,allocatable  :: rho(:),rho_it(:),rho_eff(:)
...
subroutine make_a_tov_star
  ...
  do i=1,5
     ...
     do k=1,n
     	rho_cit =rho_cr(i)
     	rho_it(k)=1.0d16+k*0.5d16
        rho_crit=rho_it(k)
     	rho_eff(k) = rho_cit - rho_cit**2/rho_crit
  ...
  ! perform the integration
     ...
     do l=1,n
     write(un,"(i8,1P10E15.6)") l,rb(l)/1.0d5,M(l)/msun,Mr(l)/msun,rho_it(l)
     enddo
     close(unit=un)  
   enddo      
